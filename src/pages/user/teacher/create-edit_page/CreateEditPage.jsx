...
const CreateEditPage = () => {
    ...
    const isButtonDisabled =
        ...
        post?.title === "" ||
        (!post?.changed &&
            !post?.postVideo?.changed &&
            !post?.videoImg?.changed);
    let { id } = useParams();
    ...
    useEffect(() => {
        ...
        if (id && isValidUUID(id)) {
            publishService
                ...
                .then((data) => {
                    publishService
                        .getFile(data?.introVideoImgLink)
                        .then((img) => {
                            dispatch(
                                setPostVideImg({
                                    changed: false,
    ...
    useEffect(() => {
        const handleBeforeUnload = (event) => {
            const message =
                "Are you sure you want to leave? The video upload process is not complete yet.";
    ...
    const setImageHandle = async () => {
        await fetch(any_image)
            ...
            .then((blob) => {
                const file = new File([blob], "loomo.png", {
                    type: "image/png",
                });
    ...
    return (
        <div className={"CreateEditPage"}>
            ...
            {showEditItem && (
                <EditText
                    setter={setter === "title" ? setPostTitle : setPostDesc}
                    num={maxChars}
            ...
            {showLanguageBar && (
                <EditLanguage
                    close={() => setShowLanguageBar(false)}
                    langauge={post.language}
                />
            )}
            {showNumberEdit && (
                <EditNumber
                    setter={setPostStudentNum}
                    close={() => setShowNumberEdit(false)}
                />
            )}
            {showPriceEdit && (
                <EditPrice
            ...
            {showListEdit && (
                <EditList
                    setter={setter === "req" ? setPostReq : setPostCourseToWho}
            ...
            {showDemoDateEdit && (
                <EditDemoDate
                    setter={setPostDemoDay}
                    close={() => setShowDemoDateEdit(false)}
                />
            )}
            {showCourseDateEdit && (
                <EditCourseDays
                    setter={setPostClassTime}
                    close={() => setShowCourseDateEdit(false)}
                />
            )}
            {showPhotoEdit && (
                <EditPostImg
                    setter={setPostVideImg}
                    close={() => setShowPhotoEdit(false)}
                />
            )}
                ...
                <div className="left">
                    ...
                    <div className="video-player-block">
                        ...
                        {showVideo && (
                            <div className="player-wrapper">
                                ...
                                />
                                <div
                                    className="btn-close close-react-player"
                    ...
                    <div className="class-lang-wrapper">
                        <div className="class-time text_32">
                            ...
                            />
                            Class time:{post.classTime.year}/
                            {post.classTime.month}/{post.classTime.day}{" "}
                            {post.classTime.hour}:{post.classTime.minute} GMT(
                            {post.classTime.gmt})
                            <br /> Days:{" "}
                            {post?.classTime?.days?.map((item) => item + "   ")}
                        </div>
                    ...
                    <div className="demo-day text_32">
                        ...
                        />
                        Demo day: {post.demoDay.year}/{post.demoDay.month}/
                        {post.demoDay.day} {post.demoDay.hour}:
                        {post.demoDay.minute} GMT({post.demoDay.gmt})
                    </div>
                </div>
